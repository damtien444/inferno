...
for video in tqdm(video_frame_dir):
    ...
    if len(set(frame_path_list) - set(output_frame_path_list)) == 0:
        ...
        print(f"Processing {video} len {len(set(frame_path_list) - set(output_frame_path_list))}")
        frame_path_list = list(set(frame_path_list) - set(output_frame_path_list))
        frame_path_list = [os.path.join(_video_frame_dir, "frame_" + frame + ".jpg") for frame in frame_path_list] 

    video_dataset = TestData(frame_path_list, face_detector="fan", max_detection=1)
    dataloader = torch.utils.data.DataLoader(video_dataset, batch_size=16, shuffle=False, num_workers=0)
    
    for batch in tqdm(dataloader):
                
        image_paths = batch["image_path"][0]
        image_ids = [os.path.basename(image_path).split(".")[0].split("_")[1] for image_path in image_paths]
        
        batch["image"] = batch["image"].cuda()
        
        # print(image_paths)
        
        latent_code, ring_size = face_rec_model.extract_latents(batch, training=False, validation=False)
        ...
        for image_idx in range(bs):
            ...
            # save json
            output_json_path = os.path.join(output_dir, video,f"{image_ids[image_idx]}.json")
